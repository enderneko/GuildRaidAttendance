...
local sheet_config = {
    ...
    columns = {
        ...
        },
        {
            ...
            key = "pr",
            width = 50,
            sort = {"pr:DESC", "name:ASC"},
        },
    },
}

for i = 1, 28 do
    tinsert(sheet_config.columns, {
        label = i,
        key = i,
        width = 30,
    })
...
-- create
---------------------------------------------------------------------
local attendanceFrame

local function Resize()
    if attendanceFrame.width and attendanceFrame.height then
        GRA_MainFrame._width = nil
        GRA_MainFrame._height = nil
        GRA_MainFrame:SetWidth(AW.ConvertPixels(5) * 2 + attendanceFrame.width)
        GRA_MainFrame:SetHeight(AW.ConvertPixels(30) * 2 + attendanceFrame.height)
    end
end

local function CreateAttendanceFrame()
    attendanceFrame = AW.CreateFrame(GRA_MainFrame_ContentHolder, "GRA_AttendanceFrame")
    attendanceFrame:SetAllPoints()

    local sheet = AW.CreateSheet(attendanceFrame, "GRA_AttendanceSheet", sheet_config, function(width, height)
        attendanceFrame.width = width
        attendanceFrame.height = height
        Resize()
    end)

    sheet:SetPoint("TOPLEFT")

    sheet:SetShownColumns()
    -- sheet:SetShownColumns({
    --     name = true,
    --     ar = true,
    --     -- ar30 = true,
    ...
    local names = {"AAA", "BBB", "CCC", "DDD", "EEE"}
    for i = 1, 30 do
        table.insert(data, {
            name = names[math.random(#names)],
            pr = AW.Round(math.random(), 3),
            ep = math.random(1, 100),
            gp = math.random(1, 100),
        })
    end

    sheet:LoadData(data)

    sheet:Sort({"pr:DESC", "name:ASC"})
end
...
local function Show(which)
    if which ~= "attendance" then
        if attendanceFrame then
            attendanceFrame:Hide()
        end
        return
    end

    if not attendanceFrame then
        CreateAttendanceFrame()
    end
    Resize()
    attendanceFrame:Show()
end
GRA.RegisterCallback("ShowTab", "ShowAttendance", Show)
