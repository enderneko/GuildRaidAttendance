...
local epgpCB = GRA.CreateCheckButton(epgpOptionsFrame, L["Enable EPGP"], nil, function(checked, cb)
	-- restore check stat
	cb:SetChecked(GRA_Config["raidInfo"]["system"] == "EPGP")
	...
	local confirm = GRA.CreateConfirmPopup(epgpOptionsFrame, epgpOptionsFrame:GetWidth()-10, text, function()
		GRA_Config["raidInfo"]["system"] = (GRA_Config["raidInfo"]["system"] == "EPGP") and "" or "EPGP"

		local enabled = GRA_Config["raidInfo"]["system"] == "EPGP"
...
-- decay static popup
---------------------------------------------------------------------
GRA.RegisterCallback("GRA_PERMISSION", "DecayEPGP_CheckPermissions", function(isAdmin)
	-- GRA_Config["lastDecayed"] = "20171130"
	if (not isAdmin) or (GRA_Config["raidInfo"]["system"] ~= "EPGP") or (GRA_Config["raidInfo"]["EPGP"][3] == 0) then return end
	...
	if GRA_Config["lastDecayed"] < current then
		...
		C_Timer.After(2, function()
			local decayP = GRA_Config["raidInfo"]["EPGP"][3]
...
epgpOptionsFrame:SetScript("OnShow", function()
	epgpCB:SetChecked(GRA_Config["raidInfo"]["system"] == "EPGP")
	ShowMask(not (GRA_Config["raidInfo"]["system"] == "EPGP"))
