...
local dkpCB = GRA.CreateCheckButton(dkpOptionsFrame, L["Enable DKP"], nil, function(checked, cb)
	-- restore check stat
	cb:SetChecked(GRA_Config["raidInfo"]["system"] == "DKP")

	local text
	if GRA_Config["raidInfo"]["system"] == "DKP" then
	...
	local confirm = GRA.CreateConfirmPopup(dkpOptionsFrame, dkpOptionsFrame:GetWidth()-10, text, function()
		GRA_Config["raidInfo"]["system"] = (GRA_Config["raidInfo"]["system"] == "DKP") and "" or "DKP"
		ShowMask(GRA_Config["raidInfo"]["system"] ~= "DKP")
		cb:SetChecked(GRA_Config["raidInfo"]["system"] == "DKP")
		-- enable/disable DKP
		GRA.SetDKPEnabled(GRA_Config["raidInfo"]["system"] == "DKP")
	end)
...
-- decay static popup
---------------------------------------------------------------------
GRA.RegisterCallback("GRA_PERMISSION", "DecayDKP_CheckPermissions", function(isAdmin)
	-- GRA_Config["lastDecayed"] = "20171130"
	if (not isAdmin) or (GRA_Config["raidInfo"]["system"] ~= "DKP") or (GRA_Config["raidInfo"]["DKP"] == 0) then return end

	local current = GRA.GetLockoutsResetDate()
	-- init
	GRA_Config["lastDecayed"] = GRA_Config["lastDecayed"] or current

	if GRA_Config["lastDecayed"] < current then
		...
		C_Timer.After(2, function()
			local decayP = GRA_Config["raidInfo"]["DKP"]
			...
			.. "\n" .. gra.colors.grey.s .. L["Yes - Decay DKP now.\nNo - Don't ask again this week."], function()
				GRA_Config["lastDecayed"] = current
				GRA.DecayDKP(decayP)
				SendChatMessage("GRA. " .. L["Decayed DKP by %d%%."]:format(decayP), "GUILD")
			end, function()
				GRA_Config["lastDecayed"] = current
...
dkpOptionsFrame:SetScript("OnShow", function()
	dkpCB:SetChecked(GRA_Config["raidInfo"]["system"] == "DKP")
	ShowMask(not (GRA_Config["raidInfo"]["system"] == "DKP"))
	decayEditBox:SetText(GRA_Config["raidInfo"]["DKP"])
end)
