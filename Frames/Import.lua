...
RefreshGuildRoster = function()
	...
	for i = 1, #roster do
		...
		if modified[playerName] then -- exists in modified, changed guild rank
			...
			-- if player already in GRA_Roster then check
			cbs_guild[playerName]:SetChecked(GRA_Roster[playerName])
		end
...
local cbsGRA_Rosteroup, currentInGroup = {}, {}
RefreshGroupRoster = function()
	...
	for i = 1, GetNumGroupMembers("LE_PARTY_CATEGORY_HOME") do
		...
		if playerName then
			...
			if not cbsGRA_Rosteroup[playerName] then
				cbsGRA_Rosteroup[playerName] = GRA.CreateCheckButton(groupFrame.scrollFrame.content, string.split("-", playerName), RAID_CLASS_COLORS[classFileName], function(checked)
					-- clicked --> modified
	...
	for i = 1, #currentInGroup do
		...
		if modified[name] then
			cbsGRA_Rosteroup[name]:SetChecked(modified[name]["checked"])
		else
			-- if player already in GRA_Roster then check
			cbsGRA_Rosteroup[name]:SetChecked(GRA_Roster[name])
		end

		cbsGRA_Rosteroup[name]:SetParent(groupFrame.scrollFrame.content) -- important!
		cbsGRA_Rosteroup[name]:Show()
		if i == 1 then
			cbsGRA_Rosteroup[name]:SetPoint("TOPLEFT", 5, 0)
		else
			cbsGRA_Rosteroup[name]:SetPoint("TOP", last, "BOTTOM")
		end
		last = cbsGRA_Rosteroup[name]
	end
end
			...
			if LGN:IsInGuild(n) then
				...
				if GRA_Config["raidInfo"]["system"] == "EPGP" then
					...
					if note then
						...
						GRA_Roster[n] = {["class"]=t.class, ["role"]="DPS", ["EP"]=ep, ["GP"]=gp}
					end
				elseif GRA_Config["raidInfo"]["system"] == "DKP" then
					...
					if note then
						...
						GRA_Roster[n] = {["class"]=t.class, ["role"]="DPS", ["DKP_Current"]=current, ["DKP_Spent"]=spent, ["DKP_Total"]=total}
					end
				else -- loot council
					GRA_Roster[n] = {["class"]=t.class, ["role"]="DPS"}
				end
			else
...
selectAllBtn:SetScript("OnClick", function()
	for i = 1, #currentInGroup do
		...
		cbsGRA_Rosteroup[name]:SetChecked(true)
