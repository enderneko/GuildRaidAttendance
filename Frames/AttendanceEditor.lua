...
attendanceEditor.header.helpBtn:HookScript("OnEnter", function()
    gra.tooltip:SetOwner(attendanceEditor.header, "ANCHOR_TOPRIGHT", 0, 1)
    gra.tooltip:AddLine(L["Attendance Editor Help"])
    gra.tooltip:AddLine(L["Double-click on the second column: "] .. "|cffffffff" .. L["Select attendance status."])
    gra.tooltip:AddLine(L["Click on the third column: "] .. "|cffffffff" .. L["Set join time (Present) / note (Absent)."])
    gra.tooltip:Show()
end)

attendanceEditor.header.helpBtn:HookScript("OnLeave", function() gra.tooltip:Hide() end)
